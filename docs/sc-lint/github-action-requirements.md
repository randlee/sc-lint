...
| `version` | yes | — | Exact released product SemVer, without a leading `v`. |
| `config-path` | no | `sc-lint.toml` | Consumer configuration passed to compatibility and operation commands. |
| `artifact-url` | no | Derived from the version, platform, and release base URL | Exact archive URL; intended for release mirrors and local fixtures. |
| `checksums-url` | no | Derived from the version and release base URL | URL of the matching checksum manifest. |
...
SHA and set `version` to the exact released product version. The Action major
tag identifies the action interface, while `version` identifies the verified
release artifact; neither is inferred from Cargo metadata or a checkout.
