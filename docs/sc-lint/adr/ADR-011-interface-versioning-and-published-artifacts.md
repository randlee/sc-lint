...
1. `sc-lint` will plan a dedicated workspace crate named `sc-lint-version`
   as the owner of interface-version checking and published interface
   artifacts.
...
6. The generated JSON sidecar is the canonical machine-readable baseline;
   main HTML reports and separate XHTML section fragments/panels are
   human-facing derivatives of that structured source.
7. The reusable HTML/XHTML template stack must expose built-in copy actions
...
8. The versioning model applies to multiple interface families:
...
9. The CLI interface family will use a versioned JSON baseline artifact
...
10. Consumer adoption guidance for `sc-lint-version` will be planned as a
   first-class product artifact, delivered through a repo-local Claude Code
   skill.
11. Marketplace advertisement for that adoption skill will be planned as a
