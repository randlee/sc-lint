...
from lint_sc_boundary import command
from lint_sc_boundary import run
...
class LintScBoundaryTests(unittest.TestCase):
    ...
    def test_run_reports_invalid_json_as_failure(
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(run(repo_root), 1)
            self.assertEqual(build_report_mock.call_args.kwargs["summary"], "sc-lint-boundary returned invalid JSON")
            print_report_mock.assert_called_once()
