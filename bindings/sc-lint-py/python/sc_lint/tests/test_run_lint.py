...
from sc_lint.run_lint import build_tasks
from sc_lint.run_lint import build_transcript
from sc_lint.run_lint import LintResult
from sc_lint.run_lint import LintTask
from sc_lint.run_lint import preview_lines_for_task
from sc_lint.run_lint import prioritize_error_lines
from sc_lint.run_lint import resolve_task_names
from sc_lint.run_lint import strip_ansi
...
"""
    ...
    def test_build_tasks_contains_expected_commands(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            self.assertEqual(tasks["deny"].command[-2:], ["-m", "sc_lint.lint_cargo_deny"])
            self.assertEqual(tasks["shear"].command[-2:], ["-m", "sc_lint.lint_cargo_shear"])
            self.assertEqual(tasks["spell"].command[-2:], ["-m", "sc_lint.lint_codespell"])
            self.assertEqual(tasks["pytests"].command[-2:], ["-m", "sc_lint.run_pytests"])
