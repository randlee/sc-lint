...
description: >
  ...
  sc-lint still exists on disk and the saved leadSessionId still matches the
...
- `SESSION_ID` still matches `leadSessionId` in
  `~/.claude/teams/sc-lint/config.json`
...
```text
SendMessage(to="<claude-teammate>", message="ping: verify sc-lint communications path")
```
...
```bash
atm teams restore sc-lint --from "$BACKUP_PATH"
```
