...
- Codex agents such as `clint` only see new ATM messages when they check
...
`team-lead@sc-lint.rand-m4` and `team-lead@sc-lint.rand-m5` are peers, each
running their own dev/`quality-mgr` under team `sc-lint`. They work
