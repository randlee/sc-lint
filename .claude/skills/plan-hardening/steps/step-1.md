# Step 1 — Plan Scope Review (`clint`)
...
```bash
atm send clint --stdin < /tmp/step-1-message.xml
```
...
- `FAIL` -> re-render and re-send Step 1 to `clint`
- if `clint` ACKs but returns no new fenced JSON, increment `round_index`,
