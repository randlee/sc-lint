...
- all sprint branches have been integrated into `integration/phase-X`
- all queued QA findings are resolved or intentionally deferred
- follow-up QA has closed the phase findings set
- a final phase-ending review is run on `integration/phase-X` before the merge to
  `develop`
...
- fix-dispatch batches sent to `clint`
- follow-up QA closeout messages from `quality-mgr`
- the final `integration/phase-X` verification report from `quality-mgr`

## Required Integration Review

Before the final merge to `develop`, `quality-mgr` must run one full review on
`integration/phase-X` after all sprint integrations are complete.
